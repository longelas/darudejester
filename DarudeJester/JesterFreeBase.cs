...
public class JesterFreeBase : BaseUnityPlugin {
    ...
    private void Awake() {
        if (Instance != null) {
            Logger.Log(LogLevel.Info, "Darude Jester instance already running");
            return;
        }
        Instance = this;
        ...
        if (!isEnabled.Value) {
            Logger.Log(LogLevel.Info, "Darude Jester disabled in config");
            return;
        }
        
        Logger.Log(LogLevel.Info, "Starting Darude Jester");
        ...
        Logger.Log(LogLevel.Info, "Darude Jester is loaded");
    }
}